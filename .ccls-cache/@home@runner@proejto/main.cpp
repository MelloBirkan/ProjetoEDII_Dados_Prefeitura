...
void imprimirFuncionario(const Funcionario& funcionario) {
    ...
    cout << "Demais elementos da remuneracao: " << funcionario.getDemais_Elementos_da_Remuneracao() << endl;
...
void leituraCSV(BSTFuncionario& bst, const std::string& nomeArquivo) {
    ...
    if (arquivo.is_open()) {
        ...
        while (std::getline(arquivo, linha)) {
            ...
            //int Excecao;
            std::string Nome_completo, Cargo_Base, Cargo_em_Comissao, Unidade, Tp_Log, Logradouro, Complemento, Jornada;
            double Remuneracao_do_Mes, Demais_Elementos_da_Remuneracao, Remuneracao_Bruta;
            int Numero;
            ...
            // Cria um objeto Funcionario e insere na árvore de busca binária
            //Funcionario funcionario(Excecao, Nome_completo, Cargo_Base, Cargo_em_Comissao, Remuneracao_do_Mes,
            //                        Demais_Elementos_da_Remuneracao, Remuneracao_Bruta, Unidade, Tp_Log, Logradouro,
            //                        Numero, Complemento, Jornada);


            Funcionario funcionario(Nome_completo, Cargo_Base, Cargo_em_Comissao, Remuneracao_do_Mes,
                                    Demais_Elementos_da_Remuneracao, Remuneracao_Bruta, Unidade, Tp_Log, Logradouro,
                                    Numero, Complemento, Jornada);
            ...
            std::cout << "Funcionario "<< Nome_completo<<" adicionado com sucesso!" << std::endl
                      << std::endl;
        ...
        std::cout << "Nao foi possivel abrir o arquivo " << nomeArquivo << std::endl;
    }
}
...
void salvarEmCSV(const std::string& nomeArquivo, const BSTFuncionario& bst) {
    ...
    arquivo << "Nome completo;Cargo Base;Cargo em Comissão;Remuneração do Mês;Demais Elementos da Remuneração;Remuneração Bruta;Unidade;Tp. Log;Logadrouro;Número;Complemento;Jornada\n";

    // Percorre a árvore em ordem
    bst.percursoInOrder([&](const Funcionario& func) {
        // Escreve as informações do funcionário no arquivo de saída
        arquivo << func.getNome_completo() << ";"
                << func.getCargo_Base() << ";"
                << func.getCargo_em_Comissao() << ";"
                << func.getRemuneracao_do_Mes() << ";"
                << func.getDemais_Elementos_da_Remuneracao() << ";"
                << func.getRemuneracao_Bruta() << ";"
                << func.getUnidade() << ";"
                << func.getTp_Log() << ";"
                << func.getLogradouro() << ";"
                << func.getNumero() << ";"
...
int main() {
    ...
    do {
        ...
        std::cout << "1 - Listar funcionarios em ordem crescente de nome"<< std::endl;
        ...
        switch (opcao) {
            case 1:
                ...
                std::cout << "Funcionários em ordem:" << std::endl
                          << std::endl;
            ...
            case 6: {
                ...
                std::cout << "Informe os demais elementos da renumeracao do funcionario: ";
                ...
                Funcionario novoFuncionario(Nome_completo, Cargo_Base, Cargo_em_Comissao, Remuneracao_do_Mes,
                                            Demais_Elementos_da_Remuneracao, Remuneracao_Bruta, Unidade, Tp_Log, Logradouro,
                                            Numero, Complemento, Jornada);
            ...
            case 23:{
                ...
                std::cin.ignore(); // descarta o caractere de nova linha deixado pelo último std::cin >>
                std::getline(std::cin, nome_completo);
                Funcionario* funcionario = bst.buscar(nome_completo);
                if (funcionario != nullptr) {
                    std::cout << "Funcionario encontrado: " << funcionario->getNome_completo() << std::endl;
                    imprimirFuncionario(*funcionario);
                } else {
