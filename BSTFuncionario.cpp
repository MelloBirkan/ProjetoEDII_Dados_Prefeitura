...
//void BSTFuncionario::inserir(const std::string& excecao, const std::string& nome_completo, const std::string& cargo_base, const std::string& cargo_comissao, double remuneracao_mes, const std::string& demais_elementos, double remuneracao_bruta, const std::string& unidade, const std::string& tp_log, const std::string& logradouro, int numero, const std::string& complemento, const std::string& jornada) {
//  Funcionario funcionario(excecao, nome_completo, cargo_base, cargo_comissao, remuneracao_mes, demais_elementos, remuneracao_bruta, unidade, tp_log, logradouro, numero, complemento, jornada);
//  inserirNo(raiz, funcionario);
//}
...
double BSTFuncionario::quemGastoSalarios() const {
    ...
    std::cout << "Maior salário: " << nomeMaiorSalario << " - R$ " << maxSalario << std::endl;
    std::cout << "Menor salário: " << nomeMenorSalario << " - R$ " << minSalario << std::endl;
...
double BSTFuncionario::gastoFuncao() const {
    ...
    std::cout << "Total de remuneração bruta do cargo base: R$ " << totalCargoBase << " (" << percentualCargoBase << "%)" << std::endl;
    std::cout << "Total de remuneração bruta do cargo em comissão: R$ " << totalCargoComissao << " (" << percentualCargoComissao << "%)" << std::endl;
...
double BSTFuncionario::gastoFuncao2() const {
    ...
    std::cout << "Total de remuneração bruta do cargo base: R$ " << totalCargoBase << " (" << ((totalCargoBase / totalSalarios) * 100) << "%)" << std::endl;
    std::cout << "Total de remuneração bruta do cargo em comissão: R$ " << totalCargoComissao << " (" << ((totalCargoComissao / totalSalarios) * 100) << "%)" << std::endl;
    std::cout << "Total de remuneração bruta: R$ " << totalSalarios << std::endl;
    ...
    for (const auto& p : totaisPorCargo) {
        ...
        std::cout << "Total de remuneração bruta do cargo " << cargo << ": R$ " << total << " (" << percentualCargo << "%)" << std::endl;
    }
...
void BSTFuncionario::calcularFuncionariosPorCargo() const {
    ...
    auto processar = [&](const Funcionario& funcionario) {
        ...
        if (!adicionou && funcionario.getCargo_em_Comissao() != "nd") {
            totaisPorCargo.push_back(std::make_pair(funcionario.getCargo_em_Comissao(), 1));
        }
    };
...
// Ordena o vetor por ordem decrescente de quantidade de funcionários
    std::sort(totaisPorCargo.begin(), totaisPorCargo.end(), [](const std::pair<std::string, int>& a, const std::pair<std::string, int>& b) {
        return a.second > b.second;
    });

    int totalFuncionarios = std::count_if(totaisPorCargo.begin(), totaisPorCargo.end(), [](const std::pair<std::string, int>& par) {
        return par.second > 0;
    });
...
// Mostra o resultado
    for (const auto& par : totaisPorCargo) {
        if (par.second > 0) {
            double percentualCargo = (par.second / static_cast<double>(totalFuncionarios)) * 100;
            std::cout << "Total de funcionários do cargo " << par.first << ": " << par.second << " (" << percentualCargo << "%)" << std::endl;
        }
    }
}
...
Funcionario* BSTFuncionario::buscar(const std::string& nome) const {
    ...
    while (no_atual != nullptr) {
        if (no_atual->funcionario.getNome_completo() == nome) {
            std::cout << "Funcionário encontrado: " << no_atual->funcionario.getNome_completo() << std::endl;
